...
@section('content')
    <div class="col-lg-12">
            ...
            <div class="alert alert-{{ $alert }} alert-dismissible">
                ...
                <p><i class="icon fas fa-info"></i> {!! $text !!}
                    ...
                    @if ($entryNow->status == 2 || $entryNow->status == 4 || $entryNow->status == 6)
                        <hr>
                        <a href="{{ url('/tbfentry/' . $entryNow->id) }}" class="btn btn-sm btn-info">Lihat Rekapitulasi Data
        ...
        <div class="card">
            <form method="post" action="{{ url('/tbfentry/storeMonthYearSelection') }}">
                <div class="card-body">
                    @csrf
                    <?php
                    ?>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="d-flex justify-content-end">
                    ...
                    @if ($entryLast !== null)
                        @php
                            $tbfLast = App\Models\TBF::where('entry_id', $entryLast->id)
                    ...
                    @if ($entryNow !== null && $entryLast !== null)
                        @php
                            $tbfNow = App\Models\TBF::where('entry_id', $entryNow->id)
                        ...
                        @endphp
                        @if (
                            ($tbfNow == $tbfLast && $entryNow->status == 0) ||
                                ($tbfNow == $tbfLast && $entryNow->status == 3) ||
                                ($tbfNow == $tbfLast && $entryNow->status == 5))
                    ...
                    @elseif($entryLast !== null)
                        @php
                            $tbfNow = '';
                        @endphp
                        @if ($entryLast->status == 6 && $tbfNow != $tbfLast)
                            <button class="btn btn-primary ml-2" data-toggle="modal" data-target="#tambah"><i
                                    class="fa fa-plus">
                                </i> </button>
                        @endif
                                            ...
                                            <div class="col-md-9">
                                                <div class="card-body">
                                                    <h5 class="card-title">
                                                        @if ($entryNow != null)
                                                            @php
                                                                $tbf = App\Models\tbf::where('entry_id', $entryNow->id)
                                                                    ->where('tanaman_id', $value->id)
                                                                    ->first();

                                                            @endphp
                                                            @if ($tbf != null && $tbf->status == 0)
                                                                <form method="post" action="{{ url('/tbfentry/reset') }}">
                                                                    @csrf
                                                                    <input type="hidden" name="tanaman_id"
                                                                        value="{{ $tbf->tanaman_id }}">
                                                            ...
                                                            @else
                                                                <a
                                                                    href="{{ url('/tbfentry/pertLuas/' . $value->id) }}">{{ $value->nama_tanaman }}</a>
                                                            @endif
                                                        @else
                                                            <a
                                                                href="{{ url('/tbfentry/pertLuas/' . $value->id) }}">{{ $value->nama_tanaman }}</a>
                                                                    ...
                                                                    @elseif($tbf->status == 5)
                                                                        ...
                                                                        @if ($tbfLast->status_tanaman == 2)
                                                                            <button class="badge badge-danger badge-sm"
                                                                                data-toggle="modal"
                                                                                data-target="#delete{{ $tbfLast->tanaman_id }}"><i
                                                    ...
                                                    @if ($tbf != null)
                                                        @if ($tbf->status == 3)
                                                            <small class="text-center text-info">Note:
                                                                {{ $tbf->catatan_dinas }}</small>
                                                        @endif
                                                        @if ($tbf->status == 5)
                                                            <small class="text-center text-info">Note:
                                                                {{ $tbf->catatan_BPS }}</small>
                                                        @endif
                                                    @elseif($tbfLast->catatan_dinas !== null && $tbfLast->status_tanaman == 2)
                                                        <small class="text-center text-info">Note:
                                                            {{ $tbfLast->catatan_dinas }}</small>
                                                    @elseif($tbfLast->catatan_BPS !== null && $tbfLast->status_tanaman == 2)
                                                        <small class="text-center text-info">Note:
                                                            {{ $tbfLast->catatan_BPS }}</small>
                                                    @endif
                                                @endif
        ...
        <div class="modal fade" id="delete{{ $value->id }}">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="post" action="{{ url('/tbfentry/' . $value->id) }}">
                        @method('delete')
                        @csrf
                        <div class="modal-body">
