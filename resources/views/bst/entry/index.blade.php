...
@section('content')
    <div class="col-lg-12">
            ...
            <div class="alert alert-{{ $alert }} alert-dismissible">
                ...
                <p><i class="icon fas fa-info"></i> {!! $text !!}
                    ...
                    @if ($entryNow->status == 2 || $entryNow->status == 4 || $entryNow->status == 6)
                        <hr>
                        <a href="{{ url('/bstentry/' . $entryNow->id) }}" class="btn btn-sm btn-info">Lihat Rekapitulasi Data
        ...
        <div class="card">
            <form method="post" action="{{ url('/bstentry/storeMonthYearSelection') }}">
                <div class="card-body">
                    @csrf
                    <?php
                    ?>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="d-flex justify-content-end">
                    ...
                    @if ($entryLast !== null)
                        @php
                            $bstLast = App\Models\BST::where('entry_id', $entryLast->id)
                    ...
                    @if ($entryNow !== null && $entryLast !== null)
                        @php
                            $bstNow = App\Models\BST::where('entry_id', $entryNow->id)
                        ...
                        @endphp
                        @if (
                            ($bstNow == $bstLast && $entryNow->status == 0) ||
                                ($bstNow == $bstLast && $entryNow->status == 3) ||
                                ($bstNow == $bstLast && $entryNow->status == 5))
                    ...
                    @elseif($entryLast !== null)
                        @php
                            $bstNow = '';
                        @endphp
                        @if ($entryLast->status == 6 && $bstNow != $bstLast)
                            <button class="btn btn-primary ml-2" data-toggle="modal" data-target="#tambah"><i
                                    class="fa fa-plus">
                                </i> </button>
                        @endif
                                            ...
                                            <div class="col-md-9">
                                                <div class="card-body">
                                                    <h5 class="card-title">
                                                        @if ($entryNow != null)
                                                            @php
                                                                $bst = App\Models\bst::where('entry_id', $entryNow->id)
                                                                    ->where('tanaman_id', $value->id)
                                                                    ->first();

                                                            @endphp
                                                            @if ($bst != null && $bst->status == 0)
                                                                <form method="post" action="{{ url('/bstentry/reset') }}">
                                                                    @csrf
                                                                    <input type="hidden" name="tanaman_id"
                                                                        value="{{ $bst->tanaman_id }}">
                                                            ...
                                                            @else
                                                                <a
                                                                    href="{{ url('/bstentry/pertLuas/' . $value->id) }}">{{ $value->nama_tanaman }}</a>
                                                            @endif
                                                        @else
                                                            <a
                                                                href="{{ url('/bstentry/pertLuas/' . $value->id) }}">{{ $value->nama_tanaman }}</a>
                                                                    ...
                                                                    @elseif($bst->status == 5)
                                                                        ...
                                                                        @if ($bstLast->status_tanaman == 2)
                                                                            <button class="badge badge-danger badge-sm"
                                                                                data-toggle="modal"
                                                                                data-target="#delete{{ $bstLast->tanaman_id }}"><i
                                                    ...
                                                    @if ($bst != null)
                                                        @if ($bst->status == 3)
                                                            <small class="text-center text-info">Note:
                                                                {{ $bst->catatan_dinas }}</small>
                                                        @endif
                                                        @if ($bst->status == 5)
                                                            <small class="text-center text-info">Note:
                                                                {{ $bst->catatan_BPS }}</small>
                                                        @endif
                                                    @elseif($bstLast->catatan_dinas !== null && $bstLast->status_tanaman == 2)
                                                        <small class="text-center text-info">Note:
                                                            {{ $bstLast->catatan_dinas }}</small>
                                                    @elseif($bstLast->catatan_BPS !== null && $bstLast->status_tanaman == 2)
                                                        <small class="text-center text-info">Note:
                                                            {{ $bstLast->catatan_BPS }}</small>
                                                    @endif
                                                @endif
        ...
        <div class="modal fade" id="delete{{ $value->id }}">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="post" action="{{ url('/bstentry/' . $value->id) }}">
                        @method('delete')
                        @csrf
                        <div class="modal-body">
